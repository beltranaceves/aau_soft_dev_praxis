...
#block(
  ...
  *Reading guide.* Each code is documented as a *strength rating* and one to three *indicators*. Ratings take one of four values: *Very strong* (5/5 interviews), *Strong* (3--4/5), *Medium* (1--2/5), or *Counter-evidence* (the pattern is *absent* in the first-person voice of every interviewee; the absence is itself a finding and is documented as such). An indicator is a short interpretive sentence followed by a verbatim interview quote and a citation. Codes that illustrate the paper's central narrative appear with multiple indicators; codes with weaker support appear with one. The Scrum Master removal case is the appendix's anchor: a single narrative instance that maps onto all three theories.
]
...
*Strength:* Strong (2/5 interviews). The anchor case is concentrated here; the second indicator provides an independent test of the same code.
...
The Scrum Master removal case illustrates Weinberg's Anti-Modularization, Lyytinen's loss of Distributed Gate-Keeping and a Peer-Network node, and Mintzberg's middle-line reversal of a Pull to Collaborate. The same passage:
